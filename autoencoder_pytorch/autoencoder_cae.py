...
"""
    CAE with same weight for input and output is not a good idea
"""
...
class AutoEncoder(nn.Module):
    ...
    def forward(self, data_in):
        data_out = F.sigmoid(torch.mm(data_in, self.weight1) + self.bias1)
        data_out = F.sigmoid(torch.mm(data_out, self.weight1.t()) + self.bias2)
        return data_out
